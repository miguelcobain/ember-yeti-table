import { _ as _applyDecoratedDescriptor, b as _initializerDefineProperty, a as _defineProperty } from '../../../../_rollupPluginBabelHelpers-CROxMPeN.js';
...
let Column = (_dec = localCopy('args.visible', true), _dec2 = localCopy('args.sortable', true), _dec3 = localCopy('args.sort'), _dec4 = localCopy('args.filterable', true), _dec5 = localCopy('args.name'), (_class = (_class2 = class Column extends Component {
  /**
  ...
  */
  ...
  get normalizedSortSequence() {
    let sortSequence1 = this.args.sortSequence;
    assert('@sortSequence must be either a comma-separated string or an array. Got `${sortSequence}.`', isArray(sortSequence1) || typeof sortSequence1 === 'string');
    if (isArray(sortSequence1)) {
      return sortSequence1;
    } else if (typeof sortSequence1 === 'string') {
      return sortSequence1.split(',').map(s1 => s1.trim());
    } else {
      return [];
    }
  }
  constructor() {
    ...
    */
    ...
    _defineProperty(this, "updateName", modifier(element1 => {
      if (!this.args.name) {
        this.name = element1.textContent.trim();
      }
    }));
  ...
  strictMode: true
}), _class2), _class2), (_descriptor = _applyDecoratedDescriptor(_class.prototype, "visible", [_dec], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: null
